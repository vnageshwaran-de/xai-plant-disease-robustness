"""
07_qual_diseased.py
Regenerate the low-stability qualitative montage on a DISEASED leaf (not 'healthy'),
so the 'prediction stays right but the explanation migrates' failure mode is shown
on an image that actually contains a lesion to anchor on (reviewer request).
"""
import os, json
import numpy as np, torch
import matplotlib; matplotlib.use("Agg")
import matplotlib.pyplot as plt
from pytorch_grad_cam import GradCAM, GradCAMPlusPlus, ScoreCAM, EigenCAM, LayerCAM
from pytorch_grad_cam.utils.model_targets import ClassifierOutputTarget
from pytorch_grad_cam.utils.image import show_cam_on_image
import common as C, model_utils as MU, xai_lib as X

RES = C.RESULTS; FIG = RES + "/figures"; BB = "efficientnet_b0"
CAMS = [("GradCAM", GradCAM), ("GradCAM++", GradCAMPlusPlus), ("ScoreCAM", ScoreCAM),
        ("EigenCAM", EigenCAM), ("LayerCAM", LayerCAM)]
CONDS = ["identity"] + X.TRANSFORMS

sub = json.load(open(f"{RES}/raw/xai_subset.json"))["images"]
recs = [json.loads(l) for l in open(f"{RES}/raw/xai_records.jsonl")
        if json.loads(l)["backbone"] == BB]
from collections import defaultdict
per = defaultdict(list)
for r in recs:
    if r["method"] in ("GradCAM", "GradCAM++", "EigenCAM", "LayerCAM"):
        per[r["img_idx"]].append(np.mean([r["stability"][t]["pearson"] for t in X.TRANSFORMS]))
score = {i: np.mean(v) for i, v in per.items()}
# diseased (not 'healthy'), has ScoreCAM (idx<12), lowest stability
cands = [i for i in score if i < 12 and "healthy" not in sub[i]["class"].lower()]
idx = min(cands, key=score.get)
print(f"diseased failure idx={idx} class={sub[idx]['class']} r={score[idx]:.3f}")

model, _ = MU.load_model(BB); tl = [MU.target_layer(model, BB)]
rec = sub[idx]; _, raw = C.load_tensor(rec["path"])
xin = X._norm(raw).unsqueeze(0)
with torch.no_grad():
    cls = int(model(xin).argmax(1))
tgt = [ClassifierOutputTarget(cls)]

fig, axes = plt.subplots(len(CAMS) + 1, len(CONDS), figsize=(2.0*len(CONDS), 2.0*(len(CAMS)+1)))
for j, cond in enumerate(CONDS):
    _, rawt = X.apply_transform(raw, cond)
    axes[0, j].imshow(rawt.permute(1, 2, 0).numpy()); axes[0, j].set_title(cond, fontsize=10); axes[0, j].axis("off")
for k, (mname, CAM) in enumerate(CAMS):
    for j, cond in enumerate(CONDS):
        xt, rawt = X.apply_transform(raw, cond)
        with CAM(model=model, target_layers=tl) as cam:
            if mname == "ScoreCAM": cam.batch_size = 128
            g = cam(input_tensor=xt, targets=tgt)[0]
        axes[k+1, j].imshow(show_cam_on_image(rawt.permute(1,2,0).numpy(), g, use_rgb=True)); axes[k+1, j].axis("off")
    axes[k+1, 0].axis("on"); axes[k+1, 0].set_xticks([]); axes[k+1, 0].set_yticks([]); axes[k+1, 0].set_ylabel(mname, fontsize=10)
fig.suptitle(f"Unstable explanations on a diseased leaf (EfficientNet-B0) --- {sub[idx]['class']}", fontsize=12)
plt.tight_layout(rect=[0, 0, 1, 0.98])
plt.savefig(f"{FIG}/fig6b_qualitative_diseased.png", dpi=140); plt.close()
print("saved fig6b_qualitative_diseased.png")
